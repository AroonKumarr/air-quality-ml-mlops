...
Provides methods to generate AQI predictions using trained models.
"""
import sys
...
from app_core.training_engine.model_registry import get_model_registry
...
class AQIPredictor:
    ...
    def get_latest_features(self, city: str = "Islamabad") -> pd.DataFrame:
        """
        Retrieves the most recent feature set for AQI prediction.

        Args:
            city (str): Name of the city (default: Islamabad)

        Returns:
            pd.DataFrame: DataFrame containing the latest features
        """
    ...
    def predict(self, features: pd.DataFrame = None, city: str = "Islamabad") -> Dict[str, Any]:
        """
        Generates AQI predictions using the trained model.

        Args:
            features (pd.DataFrame, optional): Feature DataFrame to use for prediction. If None, latest features are retrieved.
            city (str): City name for which predictions are generated (default: Islamabad)

        Returns:
            dict: Dictionary containing predicted AQI values
        """
    ...
    def _get_health_advisory(aqi: float) -> str:
        ...
        if aqi <= 50:
            return "Air quality is satisfactory. Outdoor activities can be safely enjoyed."
        elif aqi <= 100:
            ...
            return "Health alert: everyone may experience more serious health effects. Outdoor activities should be avoided."
        else:
