"""
AQI and meteorological data acquisition module.

This module retrieves historical air quality and weather measurements
from external data services and prepares them for downstream processing
in the air-quality-ml system.

Weather Data Source:
- Service: OpenWeather One Call 3.0 (Time Machine)
...
- Service: OpenWeather Air Pollution API (Historical)
...
All records are aligned and merged by timestamp to form a unified
feature set for the AQI prediction pipeline.
"""
import os
import time
import requests
...
class AQIDataFetcher:
    """
    Retrieves and combines historical weather and air quality data
    for the air-quality-ml prediction pipeline.

    - Weather features sourced from the OpenWeather One Call 3.0 API
    - Pollution features sourced from the OpenWeather Air Pollution API

    All observations are synchronized and merged by timestamp to
    produce a complete feature set for AQI modeling.
    """
   ...
   # Converts AQI values from the OpenWeatherMap index (1–5) to the US EPA AQI scale (0–500)
    ...
    def fetch_weather_current(self, city: str, country: str = "Pakistan") -> Dict[str, Any]:
        """
        Retrieves current meteorological conditions from the One Call 3.0 API.

        Returns key atmospheric attributes such as temperature, humidity,
        pressure, wind speed, wind direction, and cloud coverage.
        """
    ...
    def fetch_current_merged(self, city: str, country: str = "Pakistan") -> Dict[str, Any]:
        """
        Retrieves current observations from both weather and air quality services
        and merges them into a unified record.

        Returns a complete feature set combining meteorological
        and pollution measurements for AQI modeling.
        """
    ...
    def fetch_historical_merged(
        ...
        """
        Retrieves historical observations from both weather and air quality APIs
        and merges them by timestamp to create a unified dataset.

        This method constructs the primary training dataset for the
        air-quality-ml prediction pipeline.

        Args:
            city (str): Name of the city
            start_date (datetime or str): Start date for historical data retrieval
            end_date (datetime or str): End date for historical data retrieval
            country (str): Country name
            delay (float): Pause between API requests to comply with rate limits

        Returns:
            pd.DataFrame: Merged dataset containing weather and pollution features
        """
