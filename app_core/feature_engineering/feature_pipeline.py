...
This module generates all necessary features for training the
air-quality-ml prediction models:

1. Time-based cyclical features (hour, day, month)
2. Lagged features capturing historical pollutant values
3. Rolling statistics (mean, standard deviation, minimum, maximum)
4. AQI change rate features
5. Target variables representing future AQI or PM2.5 levels

Intended modeling approaches:

- Statistical Models: Ridge Regression, Elastic Net
- Ensemble Models: Random Forest, Gradient Boosting (XGBoost, LightGBM)
- Deep Learning Models: LSTM, GRU, Transformer-based architectures
"""
...
class FeatureEngineer:
    ...
    Generates all required features to support training of
    ...
    def add_lag_features(self, df: pd.DataFrame, 
        ...
        """
        Generates lagged versions of specified features for time series modeling.

        Args:
            df (pd.DataFrame): Input DataFrame
            columns (List[str]): Columns to create lagged features for (default: target and key pollutants)
            lags (List[int]): Lag intervals in hours (default: self.LAG_HOURS)
        """
        df = df.copy()
    ...
    def add_rolling_features(self, df: pd.DataFrame,
        ...
        """
        Computes rolling statistics (mean, standard deviation, minimum, maximum) for specified features.

        Args:
            df (pd.DataFrame): Input DataFrame
            columns (List[str]): Columns to compute rolling statistics for
            windows (List[int]): Window sizes in hours for rolling calculations
        """
        df = df.copy()
    ...
    def add_change_features(self, df: pd.DataFrame,
        ...
        """
        Calculates rate of change features for specified columns.

        Args:
            df (pd.DataFrame): Input DataFrame
            columns (List[str]): Columns for which to compute change rates
        """
        df = df.copy()
    ...
    def engineer_features(self, df: pd.DataFrame,
        ...
        Args:
            ...
            add_targets (bool): Flag to indicate whether target variables should be added
