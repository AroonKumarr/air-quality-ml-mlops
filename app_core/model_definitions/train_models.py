...
This module implements a comprehensive training pipeline for PM2.5
prediction across multiple time horizons (1h, 6h, 12h, 24h, 48h, 72h).
...
3. TREE-BASED MODELS
   ...
   - LightGBM (efficient, low-memory gradient boosting)
...
RATIONALE:
----------
- High autocorrelation (0.978 at 1h) indicates strong baseline performance
- 8,556 samples provide sufficient data for tree-based models; deep learning is feasible but limited
- 186 features require regularization (Ridge/Lasso)
- Skewed target distribution (2.39) suggests log transformation or Huber loss
- Multiple forecasting horizons necessitate separate models for each horizon
...
def load_data_from_hopsworks() -> pd.DataFrame:
    ...
    project_name = os.getenv("HOPSWORKS_PROJECT_NAME", "air-quality-ml")
...
def load_data_from_local() -> pd.DataFrame:
    ...
    path = project_root / "datasets" / "curated_data" / "islamabad_features.parquet"
